...
local pd_gridview <const> = pd.ui.gridview
...
class("ListPanel", properties, ui_panels).extends(pd_gfx.sprite)

function ui_panels.ListPanel:drawSectionHeader(listview, section, x, y, width, height)
...
function ui_panels.ListPanel:drawCell(listview, section, row, column, selected, x, y, width, height)
    if selected then
        pd_gfx.setColor(pd_gfx.kColorBlack)
		pd_gfx.setImageDrawMode(pd_gfx.kDrawModeInverted)
	else
...
function ui_panels.ListPanel:init()
    ui_panels.ListPanel.super.init(self)
    tbl.insert(panel_list, self)
...
function ui_panels.ListPanel:update()
...
function ui_panels.ListPanel:add()
    local function add_key_repeat(direction)
        if not self.key_timer then
            local function callback(timer)
                ...
                if direction == pd.kButtonDown then
            ...
            self.key_timer = pd.timer.keyRepeatTimerWithDelay(300, 50, callback)
        end
    end
    ...
    local panel_input_handlers = {
        upButtonDown = function () add_key_repeat(pd.kButtonDown) end,
        downButtonDown = function () add_key_repeat(pd.kButtonUp) end,
    ...
    ui_panels.ListPanel.super.add(self)
end

function ui_panels.ListPanel:remove()
    tbl.remove(panel_list)
    pd_input.pop()

    ui_panels.ListPanel.super.remove(self)
end

function ui_panels.ListPanel:remove_all_panels()
